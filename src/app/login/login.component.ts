...
export class LoginComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.userService
      .login(this.mainForm.get('host').value, this.mainForm.get('username').value, this.mainForm.get('password').value)
      .subscribe((data) => {
  ...
  displayHelp(texts?: DialogTexts): void {
    if (!texts) {
      texts = []
      texts.push(
        `You should use your ${this.mainForm.get('host').value} username and PrepCASE password (details below).`
      )
      texts.push({ text: '<br>', keepHtml: true })
      texts = texts.concat(this.getConfigFileHelp(this.mainForm.get('host').value))
    }
  ...
  private getConfigFileHelp(hostname: string): DialogTexts {
    ...
    texts.push({
      text: 'echo >$HOME/.prepcase.json \'{ "password": "YOUR_PASSWORD_HERE", "cesm_path": "PATH_TO_CESM", "cesm_env_script": "PATH_TO_ENV_SCRIPT", "user_scripts_dir": "SCRIPTS_DIR" }\'',
      classes: 'monospace',
    })
    texts.push(
      `"password" will be used to login to PrepCASE. It should be different than your password on "${hostname}".`
    )
    texts.push('"cesm_path" should point to your CESM installation, for example "~/CESM".')
    texts.push(
      '"cesm_env_script" (optional) is path to bash script used to set up environment before executing CESM script. Leave empty if not needed.'
    )
    texts.push(
      '"user_scripts_dir" (optional) is directory of your custom scripts. Scripts should be executable. Leave empty if not needed.'
    )
