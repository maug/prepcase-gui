...
export class UserService {
  ...
  login(host, username, password): Observable<RpcLoginResponse> {
    const res = this.jsonRpc.rpc(environment.jsonRpcUrl, 'App.login', [
      host,
      username,
      password,
  ...
  logout(): void {
    ...
    this.jsonRpc.rpc(environment.jsonRpcUrl, 'App.logout').subscribe()
  }
  ...
  private checkIfLogged(): void {
    ...
    const res = this.jsonRpc.rpc(environment.jsonRpcUrl, 'App.check_logged', []) as Observable<UserConfig | false>
