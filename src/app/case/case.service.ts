...
export class CaseService {
  ...
  loadCaseData(caseRoot: string): Promise<{ desc: string; vars: { [key: string]: string } }> {
    return new Promise((resolve, reject) => {
      this.xmlQuery(caseRoot, ['--listall']).subscribe((data) => {
        ...
        const matches = result.desc.matchAll(/^\s*(.+): (.*)$/gm)
  ...
  xmlQuery(caseRoot: string, params: string[] = []): Observable<RpcExecuteCommandResponse> {
    return this.jsonRpc.rpc(environment.jsonRpcUrl, 'App.run_script_in_case', [caseRoot, 'xmlquery', params])
  }

  xmlChange(caseRoot, key, value): Observable<RpcExecuteCommandResponse> {
    return this.jsonRpc.rpc(environment.jsonRpcUrl, 'App.run_script_in_case', [
      caseRoot,
      'xmlchange',
      [`${key}=${value}`],
    ])
  ...
  runScript(caseRoot: string, script: string, params: string[]): Observable<RpcExecuteCommandResponse> {
    return this.jsonRpc.rpc(environment.jsonRpcUrl, 'App.run_script_in_case', [caseRoot, script, params])
  }

  submitWithCylc(suitePath: string, suiteContents: string, caseDir: string): Observable<RpcExecuteCommandResponse> {
    return this.jsonRpc.rpc(environment.jsonRpcUrl, 'App.submit_with_cylc', [suitePath, suiteContents, caseDir])
  }
}
