...
export class CreateNewcaseComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.compsetsGroupsOptions = this.mainForm.get('--compset').valueChanges.pipe(
      startWith(''),
      map((value: string) => this.filterCompsetsGroups(value))
    )
    this.gridOptions = this.mainForm.valueChanges.pipe(
      startWith(''),
  ...
  onSubmit() {
    ...
    const params = this.createNewcaseParameters
      .map((item) => {
  ...
  openHelp(keyOrFormItem: string | FormItemBase<any>) {
    ...
    if (key === '--res') {
      texts.push({
        text:
          '<p>Each grid alias can be associated with two attributes:</p>' +
          '      <ul>' +
  ...
  private filterCompsets(opt: Compset[], value: string): Compset[] {
    ...
    return opt.filter(
      (item) =>
        item.longName.toLowerCase().indexOf(filterValue) !== -1 || item.name.toLowerCase().indexOf(filterValue) !== -1
    )
  }
