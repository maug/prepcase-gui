...
export class CaseListComponent implements OnInit {
  ...
  private loadCases() {
    ...
    this.caseListService.getCaseList().subscribe((data) => {
      ...
      for (const [parent, dirs] of Object.entries(data)) {
        this.userCases[parent] = dirs.map((dir) => ({
          fullPath: dir,
  ...
  openCloneDialog(data: { fullPath: string; dirName: string }) {
    this.dialog
      .open(CopyCaseDialogComponent, { minWidth: 600, disableClose: true, data })
      .afterClosed()
      .subscribe((formData) => {
        if (formData) {
          this.pleaseWaitService.show()
          this.caseListService.copyCase(formData.fullPath, formData.newPath).subscribe((res) => {
